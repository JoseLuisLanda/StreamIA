 ...
 */
export type Intent = 'greeting' | 'query' | 'ambiguous';
...
export const DEFAULT_GREETING_KEYWORDS: string[] = [
  ...
  'gracias', 'muchas gracias', 'thank you', 'thanks', 'adios', 'hasta luego', 'bye', 'chao',
];
...
/**
 * Local fast classification. Returns 'greeting' | 'query' | 'ambiguous'.
...
export function classifyIntentLocal(text: string, lists: IntentLists = {}): Intent {
  ...
  const isGreeting = hasAny(t, greetings) !== null;
  const wordCount = t.split(' ').length;

  if (isQuery) return 'query';
  if (isGreeting && wordCount <= 6) return 'greeting';
  return 'ambiguous';
}
